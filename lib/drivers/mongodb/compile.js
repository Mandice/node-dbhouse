...
function compileElement(model, key, value) {
	...
	//				}
					...
					result[cmd] = obj;
				} else if (cmd == '$regex' || cmd == '$options') {
					result[cmd] = obj;
				} else if (obj instanceof Object) {
					result[cmd] = compileElement(model, key + '.' + cmd, obj);
				} else {
...
function compile(model, conditions) {
	...
	for (var key in conditions) {
		...
		} else if (key == '$pushAll' || key == '$pullAll') {
			var arr = result[key] = {};

			for (var fieldName in obj) {
				arr[fieldName] = compileElement(model, fieldName, obj[fieldName]);
			}
		} else if (key == '$inc' || key == '$set') {
			var arr = result[key] = {};
