...
MongoDB.prototype.connect = function(options, callback) {
	...
	this.server = new Mongo.Server(options.host, options.port, { auto_reconnect: true, poolSize: options.poolSize || 10 });
...
MongoDB.prototype.update = function(dbName, tableName, data, conditions, options, callback) {
	...
	this.establish(dbName, tableName, function(db, collection) {
		...
		collection.update(execution, { $set: record }, { safe: true, multi: multi }, function(err) {
			if (callback)
				callback(err);
		});
	});
