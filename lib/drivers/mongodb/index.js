...
MongoDB.prototype.insert = function(dbName, tableName, model, data, options, callback) {
	...
	if (model) {
		if (model.getInfo('_id').type == 'UUID') { 
			if (!data._id) {
				data._id = new Buffer(uuid.v1({}, [])).toString('base64');
			}
		}
	}
...
MongoDB.prototype.update = function(dbName, tableName, model, data, conditions, options, callback) {
	...
	this.establish(dbName, tableName, model, function(db, collection) {
		...
		if (options.return_data || options.return_new_data) {
			collection.findAndModify(execution, [], updateOP, { safe: true, new: options.return_new_data || false }, function(err, data) {
				if (callback) {
					if (data) {
			...
			collection.update(execution, updateOP, { safe: true, multi: multi }, function(err) {
				if (callback)
					callback(err);
			});
		}
...
MongoDB.prototype.count = function(dbName, tableName, model, conditions, options, callback) {
	...
	this.establish(dbName, tableName, model, function(db, collection) {
		...
		collection.count(execution, selectedColumns, options, function(err, total) {
			if (err) {
				if (callback) {
					callback(err, []);
				}
...
MongoDB.prototype.createIndex = function(dbName, tableName, model, index, callback) {
	...
	function _ensureIndex(collection, index) {
		var _index = {};
	...
	function _ensureIndexes(collection, indexes) {

		for (var idx in indexes) {
			var dbIndex = indexes[idx];

			_ensureIndex(collection, dbIndex);
		}
	...
	if (!index && model) {
		...
		this.establish(dbName, tableName, model, function(db, collection) {
			...
			_ensureIndexes(collection, model.index.indexes);
